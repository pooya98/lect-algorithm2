...
def solveManhattan(initialBoard):
    ...
    # step01. PQ 선언 
    # <PQ의 원소 구조> : (예측 이동횟수, 현재까지 이동횟수, 앞으로 목표까지의 예측 이동 횟수, 현재 Board 상태, 이전 Board 상태)
    pq = PriorityQueue()

    # step02. PQ에 초기 Board 추가
    pq.put((0 + initialBoard.manhattan(), 0, initialBoard, None))

    # step03. PQ에서 minimum element 제거 후, 목표 달성 체크, 후보 Board 추가
    final_element = None

        # step03_1. PQ가 Empty가 아닐 때까지, 반복 실행

    while pq.qsize() > 0:
        # step03_2. PQ에서 Minimum element 제거
        min_element = pq.get()
        min_board = min_element[2]
        prev_real_count = min_element[1]

        # step03_3. 제거된 Minimum element가 정답인지 확인 (정답이라면, 반복 종료 / 정답이 아니라면, 진행)
        if min_board.isGoal() == True:
            final_element = min_element
            break
        
        # step03_4. 제거된 Minimum element의 이웃 board 탐색 후, 이전과 중복이 없는 board를 원소로 만들어서 PQ에 추가 
        neighbors = min_board.neighbors()
        ...
        # pointer = min_element
        # while pointer:
        #     prev_states.append(pointer[2])
        #     pointer = pointer[3]

        # for neighbor in neighbors:
        #     if neighbor not in prev_states:
        #         candidates.append(neighbor)

        prev_element = min_element[3]

        if prev_element != None:
            prev_state = prev_element[2]

            for neighbor in neighbors:
                if neighbor != prev_state:
                    pq.put((neighbor.manhattan() + prev_real_count + 1, prev_real_count + 1, neighbor, min_element))
    ...
    # step04. Root부터 정답 Board까지의 Board 상태를 결과 리스트에 저장
    result = []

    pointer = final_element
    while pointer:
        result.append(pointer[2])
        pointer = pointer[3]
