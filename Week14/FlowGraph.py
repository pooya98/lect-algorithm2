...
class BaseballElimination:
    ...
    def isEliminated(self, teamName):
        ...
        team_max_win = self.wins[teamId] + self.remaining[teamId]

        for id in range(len(self.teams)):
            if id != teamId:
                if self.wins[id] > team_max_win:
                    result.append(self.teams[id])
        ...
        # 시작 정점 -> 가장 마지막 index
        # 팀 정점   -> 팀의 index
        # 게임 정점 -> (마지막 팀 정점 index) + 1 ~ (마지막 팀 정점 index) + 게임 정점 갯수
        # 종료 정점 -> 검사하는 팀의 index

        start_id = element_num - 1
        end_id = teamId

        game_start_id = team_num
        game_end_id = team_num + (team_num - 1)*(team_num - 2) / 2 - 1

        flowNetwork = FlowNetwork(element_num)

        # [팀 정점 -> 종료 정점] 간선 추가
        for id in range(team_num):
            if id != teamId:
                newEdge = FlowEdge(id, end_id, team_max_win - self.wins[id])
                flowNetwork.addEdge(newEdge)

        # [시작 정점 -> 게임 정점] 간선 추가
        game_node_pointer = game_start_id

        for x in range(team_num):
            if x == teamId:
                continue

            for y in range(x + 1, team_num):
                if y == teamId:
                    continue

                game_num = self.against[x][y]
                newEdge = FlowEdge(start_id, game_node_pointer, game_num)
                flowNetwork.addEdge(newEdge)
        ...
        for x in range(team_num):
            ...
            for y in range(x + 1, team_num):
                ...
                newEdge1 = FlowEdge(game_node_pointer, x, float('inf'))
                newEdge2 = FlowEdge(game_node_pointer, y, float('inf'))

                flowNetwork.addEdge(newEdge1)
                flowNetwork.addEdge(newEdge2)

                game_node_pointer += 1

        FF = FordFulkerson(flowNetwork, start_id, end_id)
        ...
        ##print(flowNetwork)
        ##print("\nteamid {} -> result {}\n".format(self.teams[teamId], result))
