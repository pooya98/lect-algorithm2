...
def mstPrimEager(g):
    def include(i):
        ...
        for e in g.adj[i]:
            opponent = e.other(i)
            if not included[opponent]:
                if not indexMinPQ.contains(opponent):
                    indexMinPQ.insert(opponent, e)
                else:
                    if indexMinPQ.keyOf(opponent) > e:
                        indexMinPQ.decreaseKey(opponent, e)


    # 필요한 자료구조 초기화
    edgesInMST = [] # Stores edges selected as part of the MST
    included = [False for _ in range(g.V)] # included[v] == True if v is in the MST
    weightSum = 0  # Sum of edge weights in the MST    
    indexMinPQ = IndexMinPQ(g.V)    # MST의 후보 간선을 저장하는 IndexMinPQ

    # 시작 정점 0에 인접한 정점을 모두 PQdp 추가함.
    include(0)

    # MST를 구성하는 간선들을 저장하는 edgesInMST 리스트의 크기가 V-1이 될때까지 다음을 반복
    while len(edgesInMST) < g.V-1:
        m_key, m_index = indexMinPQ.delMin()

        edgesInMST.append(m_key)
        weightSum += m_key.weight
        include(m_index)
