...
def sap(g, aList, bList):
    ...
    queue = Queue()
    distance = [[NOT_VISITED for _ in range(g.V)] for _ in range(2)]

    for a_element in aList:
        queue.put((a_element, CONST_A))
        distance[CONST_A][a_element] = 0

    for b_element in bList:
        queue.put((b_element, CONST_B))
        distance[CONST_B][b_element] = 0

        if distance[CONST_A][b_element] == 0:
            sca, sapLength = b_element, 0

    while queue.qsize() > 0:
        v = queue.get()
        v_index, v_color = v[0], v[1]

        if distance[v_color][v_index] + 1 >= sapLength:
            break

        for w in g.adj[v_index]:
            if v_color == CONST_A:
                if distance[CONST_A][w] == NOT_VISITED:
                    distance[CONST_A][w] = distance[CONST_A][v_index] + 1
                    queue.put((w, CONST_A))

                    if distance[CONST_B][w] != NOT_VISITED:
                        new_distance = distance[CONST_A][w] + distance[CONST_B][w]

                        if new_distance < sapLength:
                            sca = w
                            sapLength = new_distance

            else:
                if distance[CONST_B][w] == NOT_VISITED:
                    distance[CONST_B][w] = distance[CONST_B][v_index] + 1
                    queue.put((w, CONST_B))
